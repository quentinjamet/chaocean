...
% plot to check the consistancy of the grid
%-- Matlab --
dla = 1/12.;
dlo = 1/11.62;% to end up with 1000 grid point in x-dir
latS = -20+dla;
...
%yy = [latS:dla:latN];
%xx = [lonW:dlo:lonE];
%[yLat_NEW,xLon_NEW]=meshgrid(yy,xx);
%[nx_MIT,ny_MIT] = size(xLon);


%-- MITgcm --
dir_in = '/tank/chaocean/qjamet/RUNS/Test_grid/';
xC_MITold = rdmds([dir_in 'grd_old/XC']);
xC_MITold = -(360 - xC_MITold);
yC_MITold = rdmds([dir_in 'grd_old/YC']);
xC_MITnew = rdmds([dir_in 'grd_new/XC']);
xC_MITnew = -(360 - xC_MITnew);
yC_MITnew = rdmds([dir_in 'grd_new/YC']);

figure(10)
clf
hold on
plot(xLon(1:1000,:),yLat(1:1000,:),'k.')
plot(xC_MITold,yC_MITold,'ro')
plot(xC_MITnew,yC_MITnew,'bo')
%plot([-98 -98],[-20 -10],'k')
%plot([-98 -90],[-20 -20],'k') 
plot([-98.0430 -98.0430],[-20.0417 -10],'k')
%plot([-98.0430 -90],[-19.9583 -19.9583],'k') 
plot([-98.0430 -90],[-20.0417 -20.0417],'k') 
grid on
set(gca,'yTick',yC_MITold(1,:))
set(gca,'xTick',xC_MITold(:,1))
axis([-98.1 -97.5 -20.1 -19.5])
title('MatLab: black dot ; MITgcm old: red circles ; MITgcm new: blue circles')

lat_Sbdy = ncread('/tank/chaocean/boundary_data/SOUTHBDY-MJM88/1961/SOUTHBDY-MJM88_y1961m12d31_gridT.nc','nav_lat');
Sbdy_latm = mean(lat_Sbdy(:));
hold on
plot([-98.0430 -90],[Sbdy_latm Sbdy_latm],'m--')
