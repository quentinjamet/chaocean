...
return
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% from integrated T,S,U,V
% generate ic for 01/01/1963 from orar config spin-up, 
% and perturbe temp. whith normal distribution of std=3.5e-3 K (similar to Germe et al., CD2017)

dir_in = '/tank/chaocean/qjamet/RUNS/ORAR/run1962/';
dir_out = '/tank/chaocean/initial_conditions_12/1963/';
dir_grd = '/tank/chaocean/grid_chaO/gridMIT/';
pick = rdmds([dir_in 'ocn/pickup'],788400);

hC = rdmds([dir_grd 'hFacC']);
ocnPts = find(hC~=0);
drF = rdmds([dir_grd 'DRF']);
dxG = rdmds([dir_grd 'DXG']);
dyG = rdmds([dir_grd 'DYG']);
vollCell = repmat(dxG.*dyG,[1 1 46]) .* repmat(drF,[1000 900 1]);

%-- generate a random normal distributed white noise --
nu = 3.5e-3;
mm = 0;
%xx = -0.1:nu/100:0.1;
%norm = normpdf(xx,mm,nu);
ic_ano = nu*randn(length(ocnPts),1);

tt = pick(:,:,2*46+1:3*46);
tt_perturb = tt;
tt_perturb(ocnPts) = tt_perturb(ocnPts) + ic_ano;

%- zonal vel -
fid = fopen([dir_out 'u_ini.bin'],'w','b');
fwrite(fid,pick(:,:,1:46),'real*4');
fclose(fid);
%- meridional vel -
fid = fopen([dir_out 'v_ini.bin'],'w','b');
fwrite(fid,pick(:,:,47:2*46),'real*4');
fclose(fid);
%- temperature -
fid = fopen([dir_out 't_ini.bin'],'w','b');
fwrite(fid,tt,'real*4');
fclose(fid);
%- perturbed temperature -
fid = fopen([dir_out 'tperturb_ini.bin'],'w','b');
fwrite(fid,tt_perturb,'real*4');
fclose(fid);
%- slat -
fid = fopen([dir_out 's_ini.bin'],'w','b');
fwrite(fid,pick(:,:,3*46+1:4*46),'real*4');
fclose(fid);
%- eta -
fid = fopen([dir_out 'eta_ini.bin'],'w','b');
fwrite(fid,pick(:,:,277),'real*4');
fclose(fid);



T = rdmds([dir_in 'T'],Inf);
S = rdmds([dir_in 'S'],Inf);
U = rdmds([dir_in 'U'],Inf);
V = rdmds([dir_in 'V'],Inf);
